  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div
          className="flex flex-wrap gap-1 bg-bone-300 p-1"
          role="tablist"
      ...
      <div className="card overflow-hidden p-0">
        ...
        <div className="border-t border-carbon-150 bg-bone-100 px-4 py-3">
...
function LeadRows({
  ...
  return (
    <>
      <tr className={cn(isNew && "bg-signal-100/70")}>
        ...
        <td className="px-4 py-2.5 font-medium text-carbon-950">
          {isNew && (
            <span
              className="mr-1.5 inline-block h-2 w-2 rounded-full bg-signal-500 align-middle"
              title="New lead"
            />
          )}
          {lead.name}
        ...
        <td className="px-4 py-2.5 text-carbon-700">
          ...
          <span className="block max-w-[180px] truncate text-xs text-carbon-500">
            {lead.email}
          </span>
        </td>
        <td className="px-4 py-2.5 text-carbon-700">{lead.suburb}</td>
        <td className="px-4 py-2.5 text-carbon-700">
        ...
        <td className="px-4 py-2.5 text-right tabular-nums">
          {lead.fromPriceAud !== null ? (
            ...
            <span className="text-xs text-carbon-500">Inspection</span>
          )}
        </td>
        <td className="px-4 py-2.5 text-xs text-carbon-700">
          {referralLabel(lead)}
        </td>
          ...
          <td colSpan={9} className="px-6 py-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <div className="md:col-span-2">
                <h3 className="mb-1 text-xs font-semibold uppercase tracking-wide text-carbon-500">
                  Issue notes
                </h3>
              ...
              <div className="space-y-2 text-sm">
                <p className="flex items-center gap-2 text-carbon-700">
                  <Mail className="h-4 w-4 text-carbon-400" aria-hidden />
                  {lead.emailedAt
                    ? `Emailed ${formatDateTime(lead.emailedAt)}`
                    : "Estimate not emailed yet"}
                </p>
                {lead.emailError && (
                  <p className=" border border-rose-200 bg-rose-50 px-2 py-1.5 text-xs text-rose-700">
                ...
                {message && (
                  <p
                    className={
                      message.ok
                        ? " border border-emerald-200 bg-emerald-50 px-2 py-1.5 text-xs text-emerald-700"
                        : " border border-rose-200 bg-rose-50 px-2 py-1.5 text-xs text-rose-700"
                    }
                  >
                    {message.text}
                  </p>
